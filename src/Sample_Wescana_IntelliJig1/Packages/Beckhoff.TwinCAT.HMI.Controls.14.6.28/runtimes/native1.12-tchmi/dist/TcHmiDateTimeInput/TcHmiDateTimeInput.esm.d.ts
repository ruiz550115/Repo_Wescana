import { Controls, EventProvider, type Locale, ObjectResolver, TcHmiControl, type FontFamily, type FontSizeUnit, type FontStyle, type FontWeight, type FourSidedCss, type HorizontalAlignment, type SolidColor } from 'Beckhoff.TwinCAT.HMI.Framework/index.esm.js';
import type { Control as TcHmiDateTimePicker } from '../TcHmiDateTimePicker/TcHmiDateTimePicker.esm.js';
import { OverrideTimeZone } from '../TcHmiDateTimePicker/TcHmiDateTimePicker.esm.js';
...
declare class TcHmiDateTimeInput extends TcHmiControl.Control {
    ...
    /** Reference to the timezone display element */
    protected __elementTimezone: HTMLElement;
    /**
    ...
    /**
     * Internal reference to the attribute "data-tchmi-override-time-zone"
     */
    protected __overrideTimeZone: OverrideTimeZone | undefined;
    ...
    destroy(): void;
    /**
     * Updates the UTC offset from the current user configuration and updates the timezone display.
     */
    protected __updateUtcOffset(): void;
    /**
     * Updates the timezone display element with the current UTC offset.
     * Hides the timezone label when the offset is 0 (UTC).
     */
    protected __processTimezoneDisplay(): void;
    /**
     * Gets the current timezone offset string.
     * @returns The timezone offset string (e.g., "+02:00").
     */
    protected __getTimezoneOffsetString(): string;
    /**
     * Converts a UTC ISO timestamp string to a local timezone ISO timestamp string for display.
     * The returned string does NOT include the timezone offset suffix (it's displayed separately).
     * @param utcIsoString The UTC ISO timestamp string.
     * @returns The local timezone ISO timestamp string without offset, or the original string if conversion fails.
     */
    protected __convertUtcToLocal(utcIsoString: string): string;
    /**
     * Converts a local timezone ISO timestamp string to a UTC ISO timestamp string.
     * @param localIsoString The local timezone ISO timestamp string (may or may not include timezone offset).
     * @returns The UTC ISO timestamp string, or the original string if conversion fails.
     */
    protected __convertLocalToUtc(localIsoString: string): string;
    /**
    ...
    /**
     * Sets the overrideTimeZone attribute and calls the associated process function (processOverrideTimeZone).
     * @preserve (Part of the public API)
     */
    setOverrideTimeZone(valueNew: OverrideTimeZone | null): void;
    /**
     * Returns the current value of overrideTimeZone.
     */
    getOverrideTimeZone(): OverrideTimeZone | undefined;
    /**
     * Process OverrideTimeZone.
     */
    protected __processOverrideTimeZone(): void;
    /**
